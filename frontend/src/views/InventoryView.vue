      ...
      >
        ...
        <el-table-column label="状态 / STATUS" width="120" align="center">
          <template #default="scope">
            <!-- 使用 el-tag 进行可视化预警 -->
            <el-tag v-if="scope.row.status === 'out_of_stock'" type="info">缺货 / OUT OF STOCK</el-tag>
            <el-tag v-else-if="scope.row.status === 'low_stock'" type="danger">库存不足 / LOW STOCK</el-tag>
            <el-tag v-else-if="scope.row.status === 'over_stock'" type="warning">库存超额 / OVER STOCK</el-tag>
